...
public sealed class UpdateServiceTests
{
    ...
    {
        string installerPath = Path.Combine(Path.GetTempPath(), "FileLocker-Setup-1.3.1.0.exe");
    ...
    public async Task DownloadInstallerPayloadAsync_ClosesTemporaryFileBeforeDigestVerification()
    {
        ...
        string installerPath = Path.Combine(root, "FileLocker-Setup-1.3.1.0.exe");
        string tempPath = Path.Combine(root, $"FileLocker-Setup-1.3.1.0.exe.{Guid.NewGuid():N}.download");
    ...
    public void TryExtractSha256DigestFromText_ExtractsMatchingInstallerDigest()
    {
        ...
        string text = $"{digest}  FileLocker-Setup-1.3.1.0.exe";
    ...
    public void TryCreateReleaseInfo_RequiresExactVersionedInstallerAsset()
    {
        var release = new GitHubRelease
        {
            ...
            [
                new GitHubReleaseAsset
                {
                    ...
                    BrowserDownloadUrl = "https://github.com/jeremymhayes/FileLocker/releases/download/v1.3.1.0/FileLocker-Setup-1.2.1.0.exe",
                    Size = 1024
                }
            ]
        };
        ...
        Assert.Contains("FileLocker-Setup-1.3.1.0.exe", failureReason, StringComparison.OrdinalIgnoreCase);
    }
    ...
    public void TryCreateReleaseInfo_UsesInstallerAndDigestSidecar()
    {
        ...
        {
            TagName = "v1.3.1.0",
            HtmlUrl = "https://github.com/jeremymhayes/FileLocker/releases/tag/v1.3.1.0",
            Body = "Release notes",
            Assets =
            [
                new GitHubReleaseAsset
                {
                    Name = "FileLocker-Setup-1.3.1.0.exe",
                    BrowserDownloadUrl = "https://github.com/jeremymhayes/FileLocker/releases/download/v1.3.1.0/FileLocker-Setup-1.3.1.0.exe",
                    Size = 1024
                },
                new GitHubReleaseAsset
                {
                    Name = "FileLocker-Setup-1.3.1.0.exe.sha256",
                    BrowserDownloadUrl = "https://github.com/jeremymhayes/FileLocker/releases/download/v1.3.1.0/FileLocker-Setup-1.3.1.0.exe.sha256",
                    Size = 96
                }
            ]
        };
        ...
        Assert.Equal("1.3.1.0", releaseInfo.DisplayVersion);
        Assert.Equal("FileLocker-Setup-1.3.1.0.exe", releaseInfo.InstallerFileName);
        Assert.EndsWith("FileLocker-Setup-1.3.1.0.exe.sha256", releaseInfo.Sha256DigestDownloadUrl, StringComparison.OrdinalIgnoreCase);
    }
